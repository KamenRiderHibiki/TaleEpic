...
bool DirectInputInit(HWND hwnd)
{
	...
	HRESULT result = DirectInput8Create(
		...
		NULL);
	/*
	dwFlags标志，描述与设备关联的协作级别。定义了以下标志：
	DISCL_BACKGROUND
		该应用程序需要后台访问。如果授予了后台访问权限，则即使关联的窗口不是活动窗口，也可以随时获取设备。
	DISCL_EXCLUSIVE
		该应用程序需要独占访问。如果授予独占访问权限，则在获取设备时，设备的其他任何实例都无法获得对该设备的独占访问权限。但是，即使另一个应用程序获得了独占访问，也始终允许对该设备进行非独占访问。在独占模式下获取鼠标或键盘设备的应用程序在收到WM_ENTERSIZEMOVE和WM_ENTERMENULOOP消息时应始终取消获取设备。否则，用户将无法操作菜单或移动窗口并调整窗口大小。
	DISCL_FOREGROUND
		该应用程序需要前台访问。如果授予了前台访问权限，则当关联的窗口移至后台时，将自动取消获取设备。
	DISCL_NONEXCLUSIVE
		应用程序需要非独占访问。对设备的访问不会干扰正在访问同一设备的其他应用程序。
	DISCL_NOWINKEY
		禁用Windows徽标键。设置此标志可确保用户不会意外退出应用程序。但是请注意，当显示默认操作映射用户界面（UI）时，DISCL_NOWINKEY无效，并且只要存在该UI，Windows徽标键就可以正常运行。
	*/
	//初始化键盘
	...
	while (!ReadFile.eof())
	{
		...
		while (workbench[i] != '\0')
		{
			if (workbench[i] >= '0'&& workbench[i] <= '9')
			{
				...
				if (!(workbench[i + 1] >= '0'&& workbench[i + 1] <= '9'))
				{
					mapdata.map[numCounter++] = -1+stoi(ThisNum);
					ThisNum = "";
				}
			}
			i++;
